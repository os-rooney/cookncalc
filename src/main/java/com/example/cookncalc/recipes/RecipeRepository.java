...
public interface RecipeRepository extends CrudRepository<Recipe, Long> {
    ...
    @Query("select r.title as Title, i.name as Zutat from Ingredient i, Recipe r, RecipeIngredient ri where ri.ingredient.id = i.id and ri.recipe.id = r.id")
    List<Object[]> getThis();

    @Query(value = "select r.title as Title, i.name as Zutat from ingredient i, recipe r, recipe_ingredient ri where ri.ingredient_id = i.id and ri.recipe_id = r.id", nativeQuery = true)
    List<Object[]> getThisNative();

    @Query(value = "select t.Rezept_ID, t.Rezept_Title, t.Markt, sum(t.Preis*t.VPE_Need) as Gesamt_Preis from (select r.id                                                                             as Rezept_ID,\n" +
            ...
            "               order by s.name) t GROUP BY t.Rezept_ID, t.Rezept_Title, t.Markt order by t.Rezept_ID, Gesamt_Preis", nativeQuery = true)
    List<Object[]> getTotalAmountPerMarket();
    ...
    @Query(value = "select t.Rezept_ID, t.Rezept_Title, t.Markt, t.Zutat, t.VPE_Need, t.Preis * t.VPE_Need as Ges_Produkt_Preis\n" +
            ...
            "        and si.ingredient_id = i.id\n" +
            "        and si.supermarket_id = s.id\n" +
            "      order by s.name) t where t.Rezept_ID = ?1\n" +
            "order by t.Rezept_ID, t.Markt, t.Zutat", nativeQuery = true)
    List<Object[]> getAmountPerIngredientPerMarketForRecipeId(Long id);
